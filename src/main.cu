...
__global__ void equilibrium_objective_kernel(double *F, double *state, void *params){
  ...
  */
  ...
  while( idx < N2 ){
    ...
    F[ 3*N2 + IDX(i,j) ] = u[IDX(i,j)]*Dxa(w,i,j) + v[IDX(i,j)]*Dya(w,i,j) - NU*LAP(w,i,j) - FORCING; //equilibrium
...
__global__ void equilibrium_action_of_Jt_kernel(double *dF, double *input, double *state, void *params){
  ...
  */
  ...
  while( idx < N2 ){
    ...
    dF[ 2*N2 + IDX(i,j) ] = dx3[IDX(i,j)] - Dxa_prod(dx4,u,i,j) - Dya_prod(dx4,v,i,j) - NU*LAP(dx4,i,j);
...
__global__ void initial_data( double *state ){
  ...
  while( idx < N2 ){
    ...
    u[idx] = cos(x+2*y + 0.4);
    v[idx] = sin(2*y);
    w[idx] = cos(2*x+0.7)*cos(y-0.5) + sin(x);
